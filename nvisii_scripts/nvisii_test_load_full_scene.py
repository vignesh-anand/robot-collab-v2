...
def main():
    ...
    # Render Images by setting camera configuration and calling render
    r._camera_configuration(at_vec=nvisii.vec3(0, 0, -5.06), 
                            up_vec=np.array([0,0,1]), 
                            eye_vec=nvisii.vec3(1.5, 0.0, -6.05), 
                            quat=np.array([1,0,0,0])
                        )
    # r.img_cntr = 0
    r.render_to_file("images/test_full_scene.png")
    r.close()
